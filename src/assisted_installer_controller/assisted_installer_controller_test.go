...
var _ = Describe("installer HostRoleMaster role", func() {
	...
	mockGetServiceOperators := func(operators []models.MonitoredOperator) {
		for index := range operators {
			if operators[index].Status != models.OperatorStatusAvailable {
				mockbmclient.EXPECT().GetClusterMonitoredOperator(gomock.Any(), gomock.Any(), operators[index].Name).Return(&operators[index], nil).Times(1)
			} else {
				mockbmclient.EXPECT().GetClusterMonitoredOperator(gomock.Any(), gomock.Any(), operators[index].Name).Return(&operators[index], nil).MinTimes(1)
			}
		}
	}
	...
	mockGetOLMOperators := func(operators []models.MonitoredOperator) {
		mockbmclient.EXPECT().GetClusterMonitoredOLMOperators(gomock.Any(), gomock.Any()).Return(operators, nil).Times(1)
	}
	...
	Context("PostInstallConfigs", func() {
		Context("waiting for cluster version", func() {
			...
			It("success", func() {
				...
				// Patching node labels
				labels := `{"cluster.ocs.openshift.io/openshift-storage": "",}`
				hosts := create3Hosts(models.HostStatusInstalled, models.HostStageDone, labels)
				mockbmclient.EXPECT().GetHosts(gomock.Any(), gomock.Any(), []string{models.HostStatusDisabled}).
					Return(hosts, nil).Times(1)
				mockk8sclient.EXPECT().ListNodes().Return(GetKubeNodes(kubeNamesIds), nil).Times(1)
				mockk8sclient.EXPECT().PatchNodeLabels(gomock.Any(), labels).Return(nil).Times(3)
			...
			It("lots of failures then success", func() {
				...
				// Patching node labels
				hosts := create3Hosts(models.HostStatusInstalled, models.HostStageDone, `{"cluster.ocs.openshift.io/openshift-storage": ""}`)
				mockbmclient.EXPECT().GetHosts(gomock.Any(), gomock.Any(), []string{models.HostStatusDisabled}).
					Return(hosts, nil).Times(1)
				mockk8sclient.EXPECT().ListNodes().Return(GetKubeNodes(kubeNamesIds), nil).Times(1)
				mockk8sclient.EXPECT().PatchNodeLabels(gomock.Any(), `{"cluster.ocs.openshift.io/openshift-storage": ""}`).Return(nil).Times(3)
			...
			It("failure", func() {
				...
				mockbmclient.EXPECT().GetClusterMonitoredOperator(gomock.Any(), gomock.Any(), consoleOperatorName).
		...
		Context("waiting for OLM", func() {
			...
			It("waiting for single OLM operator which timeouts", func() {
				By("setup", func() {
					...
					mockApplyPostInstallManifests(operators)
					mockbmclient.EXPECT().GetClusterMonitoredOLMOperators(gomock.Any(), gomock.Any()).Return(operators, nil).AnyTimes()
				})

				By("endless empty status", func() {
					mockbmclient.EXPECT().GetClusterMonitoredOperator(gomock.Any(), gomock.Any(), "lso").Return(&models.MonitoredOperator{Name: "lso", Status: ""}, nil).AnyTimes()
...
func create3Hosts(currentStatus string, stage models.HostStage, labels string) map[string]inventory_client.HostData {
	...
	return map[string]inventory_client.HostData{
		"node0": {Host: &models.Host{InfraEnvID: infraEnvId, ID: &node0Id, Labels: labels, Progress: &currentState, Status: &currentStatus}},
		"node1": {Host: &models.Host{InfraEnvID: infraEnvId, ID: &node1Id, Labels: labels, Progress: &currentState, Status: &currentStatus}},
		"node2": {Host: &models.Host{InfraEnvID: infraEnvId, ID: &node2Id, Labels: labels, Progress: &currentState, Status: &currentStatus}}}
}
